...
public class Deplacer implements Visiteur {
    ...
    public void visite(Tour tour) throws DeplacementImpossible {
        ...
        if(nouvelleCoordonnee.getX() != tour.getCoordonnees().getX() && nouvelleCoordonnee.getY() != tour.getCoordonnees().getY())
        ...
        if(tour.getCoordonnees().getX() == nouvelleCoordonnee.getX())
        {
            int increment;

            //on cherche le sens de la progression
            if(tour.getCoordonnees().getY() > nouvelleCoordonnee.getY())
                increment = -1;
            else
                increment = 1;

            //on parcours les cases entre la tour et la nouvelle position
            for(int i = (tour.getCoordonnees().getY()+increment); i < nouvelleCoordonnee.getY(); i += increment )
            {
        ...
        else if(tour.getCoordonnees().getY() == nouvelleCoordonnee.getY())
        {
            int increment;
            ...
            //on parcours les cases entre la tour et la nouvelle position
            for(int i = (tour.getCoordonnees().getX()+increment); i < nouvelleCoordonnee.getX(); i += increment )
            {
                //si la case n'est pas libre
